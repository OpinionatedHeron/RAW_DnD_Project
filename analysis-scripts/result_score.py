...
def all_scores():
    ...
    all_results = []
    for path in [
        "eval-results/context_stuffing_results_3.jsonl",
        "eval-results/rag_results_3.jsonl",
    ]:
    ...
    with open("eval-results/all_results_with_scores_3.jsonl", "a", encoding="utf-8") as f:
        for r in all_results:
            f.write(json.dumps(r) + "\n")
    print(
        f"\nCompleted scoring of {len(valid)} results. All results with scores saved to eval-results/all_results_with_scores_3.jsonl"
    )
